											...
											<tbody>
												...
												<tr <%=i + 1%>>
													...
													<td><button class="btn btn-sm btn-success btn-round">
															<a class="mr-1"
																href="<%=request.getContextPath()%>/pages/jobApplied.jsp?jobid=<%=jid%>">
																Applied</a><span class="badge badge-danger"><%=ConnectionManager.getJobAppliedCount(jid)%></span>
														</button></td>

													<td class="text-center "><a href="javascript:deleteJob('<%=jid%>')" title="Delete Job"> 
														<img src="<%=request.getContextPath()%>/theme/assets/img/delete.jpg"
...
<script>
	function deleteJob(jid){
		if(confirm("Do you really want to delete this job?")){
			$.post("<%=request.getContextPath()%>/tiles/ajax.jsp?methodId=deleteJob&jid="+jid,
					 function(data) {		
	                    data = $.trim(data);
